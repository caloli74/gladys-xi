        ...
        serverSocket.on('listening', () => {
            ...
            switch (action) {
                case 'discover':
                    ...
                    break;
                case 'setRing':
                    //pas de message, on attends juste le heartbeat pour avoir le token
                    break;
                default:
                    serverSocket.close();
                    reject("action " & action & " non supportée...");
                    return;
            }
        });
        ...
        function handleMessage(msg) {
            ...
            switch (action & "|" & parsed.cmd) {
                ...
                default:
                    ...
                    if (nbAnswers > 10) {
                        serverSocket.close();
                        reject(new Error("Pas de réponse du gateway"))
                        return;
                    }
            }
        };
